...
void MenuHandler::setErrorTimer(int time){
    errorCheckpoll = delayed_by_ms(errorCheckpoll, time);
}
...
void MenuHandler::event() {
    ...
    if(connected) {
        if (time_reached(mqttPoll)) {
            ...
            publishStatus(++msg_count, speed, autoMode ? setPressure : speed, pressure > 120 ? 120: pressure, autoMode, errorFlag,
                          co2, rh, temp);
        }
    }
    switch (MenuEventType) {
        ...
        case mainMenu: // Main menu to select the different options
            ...
            break;
        case setSpeedMode: // Set the speed of the fan
            if (rotary.rotaryturned) {
                autoMode = false;
                rotary.rotaryturned = false;
        ...
        case setPressureMode: // Set the pressure level
            if (rotary.rotaryturned) {
                autoMode = true;
                rotary.rotaryturned = false;
        ...
        case controlPressure: // Control the pressure based on the set pressure
            ...
            if(time_reached(modbus_poll)){
                ...
                display->displayStatus(modbusController->readSensor("co2"),
                                       ...
                                       modbusController->readSensor("rh") / 10,
                                       currentFanSpeed,
                                       currentPressure > 120 ? 120: currentPressure, display->wifi_signal);
                // The error flag is set if the pressure is not within the set range for 45 times
                if(currentPressure < setPressure-3){
                    errorCount++;
                } else if(currentPressure > setPressure-2 ) {
                    errorCount--;
                }
                if(errorCount >= 45){
                    errorFlag = true;
                    modbusController->writeSensor("fan", 0);
                    display->errorScreen();
                    sleep_ms(5000);
                    display->mainMenu();
                    display->selectmenu(0);
            ...
            break;
        case wifiAndMqtt: // Display the wifi and mqtt menu and return to the main menu
            if (rotary.rotaryturned) {
                rotary.rotaryturned  = false;
                menuRot += rotary.getCount();
        ...
        case CredentialsMenu: // Display the credentials menu and return to the wifi and mqtt menu
            if (rotary.rotaryturned) {
                rotary.rotaryturned  = false;
                menuRot += rotary.getCount();
                if (menuRot > 2) menuRot = 0;
        ...
        case enterCredentials: // Enter the credentials for the wifi and mqtt
            switch (key) {
                case up:
                    display->fill(0);
                    if(isWiFi){
...
void MenuHandler::seterrorcount(){
    this->errorCount = 0;
}